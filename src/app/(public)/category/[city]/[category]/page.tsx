...
export async function generateMetadata({ params }: { params: Promise<{ city: string; category: string }> }) {
  ...
  return {
    ...
    description: `Find the best ${catName} services in ${cityName}. Top-rated, verified, and ready to connect on WhatsApp. Get a response in minutes.`,
    alternates: { canonical: `https://adzbe.cloud/category/${city}/${category}` },
    openGraph: {
      title: `Best ${catName} in ${cityName} | ADZBE`,
      description: `Find the best ${catName} services in ${cityName}. Connect instantly via WhatsApp.`,
      siteName: 'ADZBE',
      locale: 'en_IN',
    },
  }
  ...
  return (
    <>
      <BreadcrumbJsonLd items={[
        ...
        { name: cityName, item: `https://adzbe.cloud/city/${city}` },
        { name: catName, item: `https://adzbe.cloud/category/${city}/${category}` },
      ]} />
      <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-gradient-to-br from-whatsapp/10 to-whatsapp/5 rounded-2xl border border-whatsapp/20 p-8 md:p-10 text-center">
            ...
            <p className="text-surface-500 mb-6 max-w-md mx-auto text-sm">
              Get listed on ADZBE and start receiving WhatsApp leads from {cityName} customers today.
            </p>
            <Link
              href="/add-business"
