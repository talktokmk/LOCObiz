...
export default async function sitemap() {
  const baseUrl = 'https://locobiz.in'
  ...
  const businessesResult = await db.execute('SELECT slug, updated_at FROM businesses')
  const businesses = businessesResult.rows as unknown as { slug: string; updated_at: string }[]

  const citiesResult = await db.execute('SELECT DISTINCT city FROM businesses')
  const cities = citiesResult.rows as unknown as { city: string }[]

  const districtsResult = await db.execute('SELECT DISTINCT state, district FROM businesses WHERE district IS NOT NULL')
  ...
  return [...staticPages, ...statePages, ...districtPages, ...cityPages, ...categoryPages, ...businessPages, ...blogPages]
}
